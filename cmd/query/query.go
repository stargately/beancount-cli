...
func NewCmdBeanQuery() *cobra.Command {
	var format string
  ...
  beancount-cli bean-query --format csv ledger.beancount "BALANCES"`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if format != "text" && format != "csv" {
				return fmt.Errorf("invalid format %q: must be \"text\" or \"csv\"", format)
			}
			...
			opts := tools.QueryOptions{Format: format}
	...
	cmd.Flags().StringVarP(&format, "format", "f", "text", `output format: "text" or "csv"`)
	return cmd
}
